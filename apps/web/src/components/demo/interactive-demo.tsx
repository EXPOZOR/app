...
import {
  ...
  getCategory,
  inferCategory,
} from "@/components/demo/interactive-demo-model";
...
import { Check, LoaderCircle, Plus, RefreshCw, Sparkles, TrendingDown, X } from "lucide-react";
...
/** Public, local-only simulation of EXPOZOR's intended expense-entry flow. */
export function InteractiveDemo() {
  ...
  const setCategorySelectRef = useCallback((node: HTMLSelectElement | null) => {
    categorySelectRef.current = node;
  ...
  const handleAdd = useCallback(() => {
    ...
    if (!description) {
      setError({ field: "name", message: "Enter a short expense description." });
      focusNameInput();
      return;
    }
    ...
    if (parsedAmount > MAX_EXPENSE) {
      setError({
        field: "amount",
        message: "Enter an amount of $1,000,000 or less for this simulation.",
      });
      requestAnimationFrame(() => amountInputRef.current?.focus());
      return;
    }
  ...
  }, [amount, clearSuggestionTimer, focusNameInput, name]);
  ...
  const handleConfirm = useCallback(() => {
    ...
    setStep("entry");
    focusNameInput();
  }, [clearSuggestionTimer, currentTx, focusNameInput, selectedCategory, spent]);
  ...
  const handleEdit = useCallback(() => {
    ...
    setStep("entry");
    focusNameInput();
  }, [clearSuggestionTimer, focusNameInput]);
  ...
  return (
    <MotionConfig reducedMotion="user">
      <section
        ...
        aria-label="Local expense simulation"
      >
            ...
            <div>
              ...
              <Input
                ...
                onChange={(event) => {
                  ...
                  setError((previous) => (previous?.field === "name" ? null : previous));
                }}
                placeholder="e.g. Coffee Shop"
                disabled={entryDisabled}
                maxLength={60}
                aria-invalid={error?.field === "name"}
                aria-describedby={error?.field === "name" ? "demo-name-error" : undefined}
              />
              {error?.field === "name" && (
                <p
                  id="demo-name-error"
              ...
              <div className="relative">
                ...
                <Input
                  ...
                  onChange={(event) => {
                    ...
                    setError((previous) => (previous?.field === "amount" ? null : previous));
                  }}
                  placeholder="4.50"
                  disabled={entryDisabled}
                  className="pl-7"
                  aria-invalid={error?.field === "amount"}
                  aria-describedby={error?.field === "amount" ? "demo-amount-error" : undefined}
                />
              </div>
              {error?.field === "amount" && (
                <p
