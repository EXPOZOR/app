...
────────────────────────────────────────────────────────────── */
const AI_ROWS = [
  { icon: "💼", merchant: "Salary — Acme Corp", amount: "+$4,200.00", cat: "Income",        color: "#34D399", pos: true  },
  { icon: "🛒", merchant: "Whole Foods Market",  amount:  "-$94.20",  cat: "Groceries",     color: "#3DDC97", pos: false },
  { icon: "☕", merchant: "Blue Bottle Coffee",  amount:   "-$6.50",  cat: "Coffee",        color: "#3DDC97", pos: false },
  { icon: "🚗", merchant: "Uber",                amount:  "-$18.40",  cat: "Transport",     color: "#60A5FA", pos: false },
  { icon: "🎬", merchant: "Netflix",             amount:  "-$15.99",  cat: "Entertainment", color: "#A78BFA", pos: false },
] as const;
...
function SnapDemo() {
  return (
    <div aria-hidden="true" style={{ marginTop: "auto", display: "flex", justifyContent: "center" }}>
      <div style={{
        position: "relative", width: "120px",
        borderRadius: "var(--radius-sm)", overflow: "hidden",
        background: "var(--bg-elev-2)", border: "1px solid var(--border)",
        padding: "12px", display: "flex", flexDirection: "column", gap: "7px",
      }}>
        {/* Receipt lines */}
        {[80, 55, 70, 40, 60].map((w, i) => (
          <div key={i} style={{
            height: "5px", borderRadius: "var(--radius-full)",
            background: i === 4 ? "var(--accent-subtle)" : "var(--bg-muted)",
            width: `${w}%`,
            border: i === 4 ? "1px solid var(--border-accent)" : "none",
          }} />
        ))}
        {/* Total line */}
        <div style={{
          display: "flex", justifyContent: "space-between",
          borderTop: "1px solid var(--border)", paddingTop: "6px", marginTop: "2px",
        }}>
          <div style={{ height: "5px", width: "30%", borderRadius: "3px", background: "var(--bg-muted)" }} />
          <div style={{ height: "5px", width: "25%", borderRadius: "3px", background: "var(--accent-subtle)" }} />
        </div>
        {/* AI result badge */}
        <motion.div
          ...
          style={{
            ...
            gap: "4px", padding: "3px 0", marginTop: "2px",
            borderRadius: "var(--radius-full)",
...
/* ──────────────────────────────────────────────────────────────
   TILE 3 — Bank sync (small, col-span-1)
   Three bank rows with animated sync indicators.
────────────────────────────────────────────────────────────── */
const BANKS = [
  { name: "Chase",        initial: "CH", synced: true  },
  { name: "Wells Fargo",  initial: "WF", synced: true  },
  { name: "Amex",         initial: "AX", synced: true  },
] as const;

function BankSyncDemo() {
  return (
    <div aria-hidden="true" style={{ marginTop: "auto", display: "flex", flexDirection: "column", gap: "6px" }}>
      {BANKS.map((b, i) => (
        <motion.div
          key={b.name}
        ...
        >
          ...
          {/* Name */}
          <span style={{ flex: 1, fontSize: "0.6875rem", fontWeight: 500, color: "var(--text-secondary)" }}>
            {b.name}
          </span>
          {/* Sync status */}
          <div style={{ display: "flex", alignItems: "center", gap: "3px" }}>
            ...
            <span style={{ fontSize: "0.5rem", fontWeight: 600, color: "var(--positive)" }}>
              Live
            </span>
          </div>
        </motion.div>
...
function BudgetsDemo() {
  return (
    <div aria-hidden="true" style={{ marginTop: "auto", display: "flex", flexDirection: "column", gap: "10px" }}>
      {ENVELOPES.map((e, i) => {
          ...
          <div key={e.label} style={{ display: "flex", flexDirection: "column", gap: "4px" }}>
            <div style={{ display: "flex", justifyContent: "space-between" }}>
              ...
              <span style={{ fontSize: "0.625rem", fontWeight: 600, color: "var(--text-secondary)" }}>
                ${e.spent}<span style={{ color: "var(--text-muted)", fontWeight: 400 }}>/${e.total}</span>
              </span>
            </div>
            <div style={{
              height: "5px", borderRadius: "var(--radius-full)",
...
────────────────────────────────────────────────────────────── */
const SUBS = [
  { name: "Adobe CC",   amount: "$54.99", days: 1,  status: "alert"    },
  { name: "Netflix",    amount: "$15.99", days: 3,  status: "upcoming" },
  { name: "Spotify",    amount: "$9.99",  days: 14, status: "ok"       },
  { name: "iCloud+",    amount: "$2.99",  days: 22, status: "ok"       },
] as const;
...
function SubRadarDemo() {
  return (
    <div aria-hidden="true" style={{ marginTop: "auto", display: "flex", flexDirection: "column", gap: "5px" }}>
      {SUBS.map((s, i) => {
        ...
        return (
          ...
          >
            <span style={{ flex: 1, fontSize: "0.6875rem", fontWeight: 600, color: "var(--text-primary)" }}>
              {s.name}
            </span>
            <span style={{ fontSize: "0.6875rem", fontWeight: 700, color: "var(--text-secondary)", fontVariantNumeric: "tabular-nums" }}>
              {s.amount}
            </span>
            {s.status !== "ok" && (
              <span style={{
      ...
      <p style={{ fontSize: "0.5625rem", color: "var(--text-muted)", margin: "4px 0 0", textAlign: "right" }}>
        $83.96/mo tracked
      </p>
    </div>
  );
...
/* ──────────────────────────────────────────────────────────────
   MARQUEE LOGO TICKER
   Inline text wordmarks — grayscale default, brand color on hover.
   Two duplicated sets → seamless @keyframes marquee-scroll 30s loop.
   Pauses on hover (aria-hidden, decorative only).
────────────────────────────────────────────────────────────── */
const BRANDS = [
  { name: "Chase",        weight: 700, ls: "-0.02em", hoverColor: "#117ACA" },
  { name: "Bank of America", weight: 700, ls: "-0.02em", hoverColor: "#E31837" },
  { name: "Wells Fargo",  weight: 800, ls: "-0.03em", hoverColor: "#CD192D" },
  { name: "Amex",         weight: 900, ls: "0.03em",  hoverColor: "#016FD0" },
  { name: "Visa",         weight: 900, ls: "-0.02em", hoverColor: "#5A74D2" },
  { name: "Mastercard",   weight: 700, ls: "-0.01em", hoverColor: "#EB5C3B" },
  { name: "Plaid",        weight: 700, ls: "-0.02em", hoverColor: "#00D64F" },
  { name: "Stripe",       weight: 700, ls: "-0.02em", hoverColor: "#635BFF" },
  { name: "PayPal",       weight: 700, ls: "-0.01em", hoverColor: "#009CDE" },
  { name: "Revolut",      weight: 700, ls: "-0.04em", hoverColor: "#0666EB" },
  { name: "Apple Pay",    weight: 600, ls: "-0.03em", hoverColor: "#d1d5db" },
  { name: "Venmo",        weight: 700, ls: "-0.02em", hoverColor: "#3D95CE" },
] as const;

function MarqueeTicker() {
  // Duplicate for seamless loop (two sets → translate -50%)
  ...
  return (
    ...
    >
      ...
      <div style={{
        ...
        background: "linear-gradient(to right, var(--bg-elev-1) 0%, transparent 12%, transparent 88%, var(--bg-elev-1) 100%)",
      }} />
      ...
      >
        {items.map((brand, i) => (
          <span
            ...
            onMouseEnter={(e) => { (e.currentTarget as HTMLSpanElement).style.color = brand.hoverColor; }}
    ...
    >
      <div className="container-site">
        ...
        <div className="bento-grid">
          ...
          {/* ① Hero tile — AI Categorization (col-span-2, row-1) */}
          <BentoCard delay={0} className="bento-hero" style={{ minHeight: "320px" }}>
            <TileHeader
              Icon={Brain}
              title="AI Categorization"
              description="Zero manual tagging. AI reads merchant names, amounts, and patterns — then assigns the right category instantly with a confidence score."
            />
            <AICatDemo />
          </BentoCard>

          {/* ② Medium tile — Snap & done (col-3, row-1) */}
          <BentoCard delay={0.06} className="bento-medium-right" style={{ minHeight: "320px" }}>
            <TileHeader
              Icon={Camera}
              title="Snap & done"
              description="Photo a receipt and it's logged, categorized, and matched to your bank statement automatically."
            />
            <SnapDemo />
          </BentoCard>

          {/* ③ Small tile — Bank sync (col-1, row-2) */}
          <BentoCard delay={0.1} className="bento-small">
            <TileHeader
              Icon={Building2}
              title="10,000+ banks"
              description="Every account, live. No manual imports ever."
            />
            <BankSyncDemo />
          </BentoCard>

          {/* ④ Small tile — Budgets (col-2, row-2) */}
          <BentoCard delay={0.14} className="bento-small">
            <TileHeader
              Icon={PieChart}
              title="Envelope budgets"
              description="Every dollar has a job. Unused budget rolls forward intelligently."
            />
            <BudgetsDemo />
          </BentoCard>

          {/* ⑤ Medium tile — Subscription radar (col-3, row-2) */}
          <BentoCard delay={0.18} className="bento-medium-right">
            <TileHeader
              Icon={Radio}
              title="Subscription radar"
              description="Spots recurring charges — especially the ones you forgot — and alerts you before they hit."
            />
            <SubRadarDemo />
          </BentoCard>

          {/* ⑥ Full-width tile — Bank marquee (row-3) */}
          <BentoCard delay={0.22} className="bento-full">
            <div style={{ marginBottom: "1.25rem" }}>
              <h3
                style={{
              ...
              >
                Connects with 10,000+ banks and apps
              </h3>
              <p style={{ fontSize: "0.8125rem", color: "var(--text-muted)", textAlign: "center", margin: 0 }}>
                Powered by Plaid · Supports all major institutions worldwide
              </p>
            </div>
            <MarqueeTicker />
          </BentoCard>
